    ...
    try:
      ...
      for review in data['reviews']:
        if 'word_id' not in review or 'correct' not in review:
          return jsonify({"error": "Each review must contain word_id and correct fields"}), 400
        
        # Check if word exists
        cursor.execute('SELECT id FROM words WHERE id = ?', (review['word_id'],))
        if not cursor.fetchone():
          return jsonify({"error": f"Word with id {review['word_id']} not found"}), 400
      ...
      for review in data['reviews']:
        cursor.execute('''
          ...
          VALUES (?, ?, ?, datetime('now'))
        ''', (id, review['word_id'], int(review['correct'])))
